...
public class phase3bperez35 {
                ...
                if (op1 == 2) {
                    ...
                    for (int j = 0; j < movieTuple.length; j++) {
                        if (j == 1) {
                            ...
                            stmt.setInt((j + 1), ksus);     stmt.addBatch();
                        }
                        else if (j == 3) {
                            float fsus = Float.parseFloat(movieTuple[j]);
                            stmt.setFloat((j + 1), fsus);   stmt.addBatch();
                        }
                        else { 
                            stmt.setString((j + 1), movieTuple[j]);     stmt.addBatch();
                        }
                    }
                    stmt.executeBatch();
                    c.commit();
                    ...
                    for (int k = 0; k < searchOps.length; k++) {System.out.println(searchOps[k]);}
        ...
        try {
            ...
            if (selectP == 1) {
                ...
                while(rs2.next()) {
                    ...
                    if (internalUserN.length() == ctP1 && titlematch.length() == ctP2) {
                        searchMatch1 = true;    searchMatch2 = true;    break;
                    }
                    else if (internalUserN.length() == ctP1 && titlematch.length() != ctP2) {
                        searchMatch1 = true;    searchMatch2 = false;   break;
                    }
                ...
                if (searchMatch2 == false) {
                    ...
                    //String sql4 = "UPDATE userAcc SET u_numpurchase = " + numP + " WHERE u_username = '" + userNameP + "'";
                    //String sql5 = "UPDATE purchases SET p_numpurchase = " + numP + "WHERE p_username = '" + userNameP + "'";
                    //String sql6 = "UPDATE userAcc SET u_acctbal = " + currBal + " WHERE u_username = '" + userNameP + "'";
                    //String sql7 = "UPDATE purchases SET p_acctbal = " + currBal + " WHERE p_username = '" + userNameP + "'";

                    String sql4 = "UPDATE ? SET ? = ? WHERE ? = ?";
                    PreparedStatement pSt4 = c.prepareStatement(sql4);
                    pSt4.setString(1, "userAcc");       pSt4.setString(2, "u_numpurchase");     pSt4.setInt(3, numP);  
                    pSt4.setString(4, "u_username");    pSt4.setString(5, userNameP);           pSt4.addBatch();

                    pSt4.setString(1, "purchases");     pSt4.setString(2, "p_numpurchase");     pSt4.setInt(3, numP);  
                    pSt4.setString(4, "p_username");    pSt4.setString(5, userNameP);           pSt4.addBatch();

                    pSt4.setString(1, "userAcc");       pSt4.setString(2, "u_acctbal");         pSt4.setInt(3, currBal);  
                    pSt4.setString(4, "u_username");    pSt4.setString(5, userNameP);           pSt4.addBatch();

                    pSt4.setString(1, "purchases");     pSt4.setString(2, "p_acctbal");         pSt4.setInt(3, currBal);  
                    pSt4.setString(4, "p_username");    pSt4.setString(5, userNameP);           pSt4.addBatch();

                    pSt4.executeBatch();    c.commit();     pSt4.close();
                    ...
                    System.out.println("FILM " + _title + " HAS ALREADY BEEN ADDED TO ACCOUNT " + userNameP + "!!!");
                }
    ...
    private void watchList(String usernameX) {
        ...
        for (int x = 0; x < watchLMenu.length; x++) {System.out.println(watchLMenu[x]);}
        int selectX = 0;
        try {
            ...
            if (selectX == 1) {
                ...
                PreparedStatement stmtX = c.prepareStatement(sql);
                ...
                String film = uX.next();
                String sql = "SELECT x_title FROM watched WHERE = '" + usernameX + "'";
                ...
                if (addFilm == true) {
                    ...
                    PreparedStatement wew = c.prepareStatement(sqlAlpha);

                    wew.setString(1, _title);       wew.setString(2, usernameX);        wew.executeUpdate(sqlAlpha);
                    c.commit();     wew.close();
                }
            }
            else if (selectX == 3) {
                ...
                wewX.setString(1, filmA);       wewX.setString(2, usernameX);        wewX.executeUpdate(sqlAlpha);
                c.commit();     wewX.close();
            }
        }
        catch (Exception e) {
    ...
    private void wishList(String usernameW) {
        ...
        try {
            ...
            if (selectW == 1) {
                ...
                wewX.setString(1, filmA);       wewX.setString(2, usernameW);       wewX.execute(sqlAlpha);
                c.commit();         wewX.close();
            }
        }
        catch (Exception e) {
    ...
    public static void main (String args[]) {
        ...
        entersus.openConnection("data-proj-test1.sqlite");
...
/*
    FUNCTION (and SUBFUNCTIONS/SUBQUERIES):         1st Draft:      1st Testing:    2nd Testing:     
    userAcc()                                       COMPLETE        YES (x)         YES (O)
    createAcct()                                    COMPLETE        NO              YES (O)
    movieDB()                                       COMPLETE        NO              YES (O-)
    purchaseList()                                  COMPLETE        NO              YES (O--)
    watchList()                                     COMPLETE        NO              NO (X)
    wishList()                                      COMPLETE        NO              NO (X)
    scoreList()                                     COMPLETE        NO              NO ()
    actorList()                                     PENDING         NO              NO ()
